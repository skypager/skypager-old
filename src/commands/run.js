...
export async function run() {
  ...
  const matcher = new RegExp(`scripts.*${script.toLowerCase()}.js$`)

  const matchingFiles = this.runtime.fileManager.fileIds.filter(
    fileId =>
      matcher.test(fileId) ||
      fileId === `${script}.js` ||
      (fileId.endsWith(`${script}.js`) && fileId.match(/scripts/))
  )
